<template>
	<view class="content">
		...
		>
			<view class="box">
				<view :class="{text_item:true, active: isFloat === index}"
				v-for="(item,index) in showFloat"
				:key='index'
				@click="changeFloat(item,index)"
				>
					{{item}}
				</view>
			</view>
	...
	export default {
		data () {
			return {
				...
				isFloat: 1,
				memberType: '直推会员',
				isFloatShow: false, // 是否展示选择会员类型弹窗
				joinList: [
					{
						store: '屈臣氏(成都百盛店)',
